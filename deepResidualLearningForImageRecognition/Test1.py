...
def model(x, y_, reuse):
    ...
    with tf.variable_scope("model", reuse=reuse):
        ...
        net = DenseLayer(net, n_units=192, act=tf.nn.relu,
                         W_init=W_init2, b_init=b_init2, name='d3relu')  # output: (batch_size, 192)
        net = DenseLayer(net, n_units=192, act=tf.nn.relu,
                         W_init=W_init2, b_init=b_init2, name='d4relu')  # output: (batch_size, 192)
        net = DenseLayer(net, n_units=192, act=tf.nn.relu,
                         W_init=W_init2, b_init=b_init2, name='d5relu')  # output: (batch_size, 192)
        net = DenseLayer(net, n_units=192, act=tf.nn.relu,
                         W_init=W_init2, b_init=b_init2, name='d6relu')  # output: (batch_size, 192)
        net = DenseLayer(net, n_units=192, act=tf.nn.relu,
                         W_init=W_init2, b_init=b_init2, name='d7relu')  # output: (batch_size, 192)
        net = DenseLayer(net, n_units=192, act=tf.nn.relu,
                         W_init=W_init2, b_init=b_init2, name='d8relu')  # output: (batch_size, 192)
        net = DenseLayer(net, n_units=192, act=tf.nn.relu,
                         W_init=W_init2, b_init=b_init2, name='d9relu')  # output: (batch_size, 192)
        net = DenseLayer(net, n_units=192, act=tf.nn.relu,
                         W_init=W_init2, b_init=b_init2, name='d10relu')  # output: (batch_size, 192)
        net = DenseLayer(net, n_units=192, act=tf.nn.relu,
                         W_init=W_init2, b_init=b_init2, name='d11relu')  # output: (batch_size, 192)
...
def model_batch_norm(x, y_, reuse, is_train):
    ...
    with tf.variable_scope("model", reuse=reuse):
        ...
        net = DenseLayer(net, n_units=192, act = tf.nn.relu,
                    W_init=W_init2, b_init=b_init2, name='d2relu')           # output: (batch_size, 192)
        net = DenseLayer(net, n_units=96, act=tf.nn.relu,
                         W_init=W_init2, b_init=b_init2, name='d3relu')  # output: (batch_size, 192)
        net = DenseLayer(net, n_units=48, act=tf.nn.relu,
                         W_init=W_init2, b_init=b_init2, name='d4relu')  # output: (batch_size, 192)
        net = DenseLayer(net, n_units=24, act=tf.nn.relu,
                         W_init=W_init2, b_init=b_init2, name='d5relu')  # output: (batch_size, 192)
        net = DenseLayer(net, n_units=12, act=tf.nn.relu,
                         W_init=W_init2, b_init=b_init2, name='d6relu')  # output: (batch_size, 192)
        net = DenseLayer(net, n_units=12, act=tf.nn.relu,
                         W_init=W_init2, b_init=b_init2, name='d7relu')  # output: (batch_size, 192)
        net = DenseLayer(net, n_units=12, act=tf.nn.relu,
        ...
        net = DenseLayer(net, n_units=12, act=tf.nn.relu,
                         W_init=W_init2, b_init=b_init2, name='d9relu')  # output: (batch_size, 192)
        net = DenseLayer(net, n_units=10, act = tf.identity,
                    W_init=tf.truncated_normal_initializer(stddev=1/12.0),
                    name='output')                                          # output: (batch_size, 10)
        y = net.outputs
...
train_loss_set = np.arange()
test_loss_set =np.arange()
for epoch in range(n_epoch):
    ...
    if epoch + 1 == 1 or (epoch + 1) % print_freq == 0:
        ...
        train_loss_set.__add__(train_loss/n_batch)
        ...
        test_loss_set.append(test_loss/n_batch)

from matplotlib import pyplot as plt
import numpy as np
l1 = train_loss_set.__len__();
l2 = test_loss_set.__len__();

x = np.arange(0, l1, 1)#x轴上的点，0到2之间以0.01为间隔
y = np.arange(0,l2,1)
plt.plot(y,test_loss_set)
plt.plot(x, train_loss_set)  # 画图
plt.xlabel('iter ')  # x轴标签
plt.ylabel('train loss ')  # y轴标签
plt.title('About as simple as it gets, folks')  # 图的标签
plt.grid(True)  # 产生网格
plt.savefig("train1.png")  # 保存图像
plt.show()  # 显示图像
