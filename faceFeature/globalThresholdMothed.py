...
if __name__ == '__main__':
    ...
    for imgCounter in range(start,end):
        print("==========================="+str(imgCounter))
        inputImg = cv2.imread("imageTailor/1 ("+str(imgCounter)+").jpg")
        if inputImg is None:
            continue
        ...
        Y = [0]
        cv2.imwrite("globalResult/"+str(imgCounter)+".jpg",temp)
        cv2.waitKey(30)
    cv2.waitKey(0)
