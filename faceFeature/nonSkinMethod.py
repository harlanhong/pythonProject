...
def RemoveSelectRegion(src,AreaHigh,AreaLow,CheckMode,NeiborMode):
    ...
    return dst
#阈值化
def myThreshold(imgGray,thresh,globalAvg=0,localAvg=0):
    ...
    #     for i in range(sp[0]):
    #         for j in range(sp[1]):
    #             if imgGray[i,j]>thresh and imgGray[i,j]>globalAvg:
    #                 dst[i,j] = 255
    #             else:
    #                 dst[i,j] = 0
    # else:
...
def computeEdgesPoint(img):
    ...
    if temp > 0 and temp<=0.05:
        ...
        return temp
#分区结合边缘检测的方法来进行二值化
def divisionThreshold(imgFace,imgCanny,imgBGR):
    theta = meanBrightness(imgGRAY=imgFace)
    ...
    for i in range(divisionCount):
        for j in range(divisionCount):
            ...
            alpha = meanBrightness(imgGRAY=imgSegment[i][j])
            if alpha !=0:
                ...
                if alpha>theta:
                    #尽量往上
                    imgSegment[i][j] = myThreshold(imgSegment[i][j],(1+edgesPointPercent)*gamma*(lamda*max_th+bata*min_th),globalAvg=theta,localAvg=alpha)
                else:
                    #尽量往下
                    imgSegment[i][j] = myThreshold(imgSegment[i][j],(1+edgesPointPercent)*gamma*(lamda1*max_th+bata1*min_th),globalAvg=theta,localAvg=alpha)
    for i in range(divisionCount):
        for j in range(divisionCount):
            dst[i*new_h:(i+1)*new_h,j*new_w:(j+1)*new_w]=imgSegment[i][j]
    ret,dst = cv2.threshold(dst,150,255,cv2.THRESH_BINARY)
    return dst,newFace,newBGR
#头发处理
...
def processImg(img):
    ...
    imgFace_thresh,newFace,newBGR = divisionThreshold(imgFace=imgFace_Gray,imgCanny=canny,imgBGR = imgFace)
